...
const DetailGoldPage = ({ date }: { date: string }) => {
  ...
  const necessary_points = Number(process.env.NEXT_PUBLIC_GOLD_POINTS)
      ...
      onConfirm: async () => {
        try {
          ...
          if (prediction_id) {
            ...
            router.push(`/prediction/gold/${prediction_id}`)
          }
        } catch (err) {
  ...
  return (
    <div className="w-full h-full grow p-2 flex flex-col items-center justify-center">
      <div className="w-full max-w-[500px] mx-auto p-8 bg-gradient-to-br from-black via-gray-900 to-gray-800 rounded-xl shadow-lg flex flex-col items-center">
        <h2 className="text-2xl sm:text-3xl font-bold text-m-gold mb-6 shadow-sm">【LOTO6 | GOLD】購入</h2>
